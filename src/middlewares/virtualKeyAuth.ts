...
export async function virtualKeyAuth(c: Context, next: Next) {
  ...
  try {
    ...
    if (!matchedKey) {
      console.warn(
        `[${timestamp}] [VirtualKeyAuth] [WARN] Invalid virtual key provided`
      );
      return c.json(
        {
          status: 'failure',
    ...
    if (matchedKey.expiresAt && new Date(matchedKey.expiresAt) < new Date()) {
      console.warn(
        `[${timestamp}] [VirtualKeyAuth] [WARN] Expired virtual key used: ${matchedKey.id}`
      );
      return c.json(
        {
          status: 'failure',
    ...
    if (!workspace) {
      console.error(
        `[${timestamp}] [VirtualKeyAuth] [ERROR] Workspace not found for virtual key: ${matchedKey.id}`
      );
      return c.json(
        {
          status: 'failure',
    ...
    if (!providerKey) {
      console.error(
        `[${timestamp}] [VirtualKeyAuth] [ERROR] Provider key not found for virtual key: ${matchedKey.id}`
      );
      return c.json(
        {
          status: 'failure',
    ...
    console.error(
      `[${timestamp}] [VirtualKeyAuth] [ERROR] Authentication error:`,
      error.message
    );
    return c.json(
      {
...
export async function validateRequestedModel(c: Context, next: Next) {
  ...
  try {
    ...
    console.warn(
      `[${timestamp}] [VirtualKeyAuth] [WARN] Could not parse request body for model validation:`,
      error.message
    );
    return next();
  }
