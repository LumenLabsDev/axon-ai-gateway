...
export async function adminKeyAuth(c: Context, next: Next) {
  ...
  try {
    ...
    if (!adminKey) {
      console.warn(
        `[${timestamp}] [AdminKeyAuth] [WARN] No admin key provided`
      );
      return c.json(
        {
          status: 'failure',
    ...
    if (!matchedKey) {
      console.warn(
        `[${timestamp}] [AdminKeyAuth] [WARN] Invalid admin key provided`
      );
      return c.json(
        {
          status: 'failure',
    ...
    if (matchedKey.expiresAt && new Date(matchedKey.expiresAt) < new Date()) {
      console.warn(
        `[${timestamp}] [AdminKeyAuth] [WARN] Expired admin key used: ${matchedKey.id}`
      );
      return c.json(
        {
          status: 'failure',
    ...
    console.error(
      `[${timestamp}] [AdminKeyAuth] [ERROR] Authentication error:`,
      error.message
    );
    return c.json(
      {
