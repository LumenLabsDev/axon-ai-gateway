...
export const providerKeys = sqliteTable(
  ...
  (table) => ({
    workspaceProviderIdx: index('provider_keys_workspace_provider_idx').on(
      table.workspaceId,
      table.provider
    ),
  })
 ...
 */
export const adminKeys = sqliteTable(
  'admin_keys',
  {
    id: text('id')
      .primaryKey()
...
export const virtualKeys = sqliteTable(
  ...
  (table) => ({
    ...
    providerKeyIdx: index('virtual_keys_provider_key_idx').on(
      table.providerKeyId
    ),
  })
);
...
export const promptVersions = sqliteTable(
  'prompt_versions',
  {
    ...
    status: text('status', {
      enum: ['draft', 'development', 'staging', 'production'],
    })
      .notNull()
      .default('draft'),
    createdBy: text('created_by').references(() => users.id),
 ...
 */
export const promptPartials = sqliteTable(
  'prompt_partials',
  {
    id: text('id')
      .primaryKey()
 ...
 */
export const rateLimitUsage = sqliteTable(
  'rate_limit_usage',
  {
    id: text('id')
      .primaryKey()
  ...
  (table) => ({
    virtualKeyWindowIdx: index('rate_limit_usage_virtual_key_window_idx').on(
      table.virtualKeyId,
      table.windowStart
    ),
  })
 ...
 */
export const workspaceGuardrails = sqliteTable(
  'workspace_guardrails',
  {
    ...
    virtualKeyId: text('virtual_key_id').references(() => virtualKeys.id, {
      onDelete: 'cascade',
    }), // optional: bind to specific key
    mode: text('mode', { enum: ['block', 'observe'] })
      .notNull()
      .default('observe'),
    createdAt: integer('created_at', { mode: 'timestamp' })
      .notNull()
  ...
  (table) => ({
    workspaceIdx: index('workspace_guardrails_workspace_idx').on(
      table.workspaceId
    ),
    guardrailIdx: index('workspace_guardrails_guardrail_idx').on(
      table.guardrailId
    ),
  })
);
