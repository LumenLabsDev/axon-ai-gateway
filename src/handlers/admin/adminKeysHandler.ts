...
export async function listAdminKeys(c: Context) {
  ...
  try {
    const keys = await db.select().from(adminKeys);
    ...
    console.log(
      `[${timestamp}] [AdminKeysHandler] [INFO] Listed ${maskedKeys.length} admin keys`
    );
    ...
    console.error(
      `[${timestamp}] [AdminKeysHandler] [ERROR] Failed to list admin keys:`,
      error.message
    );
    return c.json(
      {
...
export async function getAdminKey(c: Context) {
  ...
  try {
    ...
    if (!key) {
      console.warn(
        `[${timestamp}] [AdminKeysHandler] [WARN] Admin key not found: ${id}`
      );
      return c.json(
        {
          status: 'failure',
    ...
    console.log(
      `[${timestamp}] [AdminKeysHandler] [INFO] Retrieved admin key: ${id}`
    );
    ...
    console.error(
      `[${timestamp}] [AdminKeysHandler] [ERROR] Failed to get admin key:`,
      error.message
    );
    return c.json(
      {
...
export async function createAdminKey(c: Context) {
  ...
  try {
    ...
    console.log(
      `[${timestamp}] [AdminKeysHandler] [INFO] Created admin key: ${created.id} (${created.name})`
    );
    ...
    return c.json(
      {
        ...
        message:
          'Admin key created. Save the plainKey now - it will not be shown again.',
      },
      201
    );
  } catch (error: any) {
    console.error(
      `[${timestamp}] [AdminKeysHandler] [ERROR] Failed to create admin key:`,
      error.message
    );
    return c.json(
      {
...
export async function updateAdminKey(c: Context) {
  ...
  try {
    ...
    console.log(
      `[${timestamp}] [AdminKeysHandler] [INFO] Updated admin key: ${id}`
    );
    ...
    console.error(
      `[${timestamp}] [AdminKeysHandler] [ERROR] Failed to update admin key:`,
      error.message
    );
    return c.json(
      {
...
export async function deleteAdminKey(c: Context) {
  ...
  try {
    ...
    console.log(
      `[${timestamp}] [AdminKeysHandler] [INFO] Deleted admin key: ${id}`
    );
    ...
    console.error(
      `[${timestamp}] [AdminKeysHandler] [ERROR] Failed to delete admin key:`,
      error.message
    );
    return c.json(
      {
