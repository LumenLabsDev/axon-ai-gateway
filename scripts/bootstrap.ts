...
async function bootstrap() {
  ...
  try {
    ...
    // Interactive prompts
    console.log(
      '📝 Please provide the following information (press Enter to use defaults):\n'
    );
    const workspaceName = await promptUser(
      'Workspace name',
      'Default Workspace'
    );
    ...
    console.log(
      '      Body: { "workspaceId": "YOUR_WORKSPACE_ID", "name": "OpenAI Key", "provider": "openai", "apiKey": "sk-..." }'
    );
    console.log('   4. Create virtual keys linked to your provider keys:');
    console.log('      POST /v1/admin/virtual-keys');
    console.log(
      '      Body: { "workspaceId": "YOUR_WORKSPACE_ID", "providerKeyId": "PROVIDER_KEY_ID", "name": "My App Key", "allowedModels": ["gpt-4o"] }'
    );
    console.log('   5. Use virtual keys to access the gateway:');
    console.log(
      '      curl -H "x-axon-api-key: YOUR_VIRTUAL_KEY" http://localhost:8787/v1/chat/completions\n'
    );
  } catch (error: any) {
